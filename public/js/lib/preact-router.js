...
function pathRankSort(a, b) {
	...
	if (aAttr.default || aAttr.path instanceof RegExp) { return 1; }
	if (bAttr.default || bAttr.path instanceof RegExp) { return -1; }
...
function getMatchesFromRegex(attrs, url, path){
	var obj = {};
	...
	Router.prototype.getMatchingChildren = function getMatchingChildren(children, url, invoke) {
		return children.slice().sort(pathRankSort).map(function (vnode) {
			var attrs = vnode.attributes || {},
				path = attrs.path,
				matches = (path instanceof RegExp) ? getMatchesFromRegex(attrs, decodeURIComponent(url), path) : exec(decodeURIComponent(url), path, attrs);
			if (matches) {
				if (invoke !== false) {
					var newProps = { url: decodeURIComponent(url), matches: matches };
